...
class InsPartnerLedger(models.TransientModel):
    ...
    def write(self, vals):
        ...
        if vals.get('partner_category_ids'):
            vals.update(
                {'partner_category_ids': vals.get('partner_category_ids')})
        if vals.get('partner_category_ids') == []:
    ...
    def action_pdf(self):
        ...
        return self.env.ref(
            ...
            '.action_print_partner_ledger').with_context(landscape=False).report_action(
    ...
    def get_xlsx_report(self, data, response):
        ...
        if account_lines:
            for line in account_lines:
                row_pos += 1
                sheet.merge_range(row_pos, 0, row_pos, 4,
                                  account_lines[line].get('name'), line_header)
