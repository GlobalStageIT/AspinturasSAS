...
class InsGeneralLedger(models.TransientModel):
    ...
    view_tax_base_amount = fields.Boolean(
        string="Tax base amount", default=False)
    @api.model
    ...
    def process_filters(self):
        ...
        filters['tax_view'] = data.get('tax_view', False)
    ...
    def build_detailed_move_lines(self, offset=0, account=0, fetch_range=FETCH_RANGE):
        ...
        if (int(offset_count / fetch_range) == 0) and data.get('initial_balance'):
            sql = ('''
                    SELECT 
                        COALESCE(SUM(l.tax_base_amount),0) AS tax_base_amount,
        ...
        sql = ('''
                SELECT
                    ...
                    COALESCE(l.tax_base_amount,0) AS tax_base_amount,
                ...
                JOIN account_journal j ON (l.journal_id=j.id)
                WHERE %s
                GROUP BY l.id, l.account_id, l.date, j.code, l.currency_id, l.amount_currency, l.name, m.id, m.name, c.rounding, cc.id, cc.rounding, cc.position, c.position, c.symbol, cc.symbol, p.name, p.vat, anl.name
                ORDER BY %s
                OFFSET %s ROWS
                FETCH FIRST %s ROWS ONLY
        ...
        if ((count - offset_count) <= fetch_range) and data.get('initial_balance'):
            ...
            for row in cr.dictfetchall():
                ...
                row['company_currency_id'] = currency_id.id
                move_lines.append(row)
        return count, offset_count, move_lines, data.get('tax_view')
    ...
    def process_data(self):
        ...
        for account in account_ids:
            ...
            sql = ('''
                ...
                JOIN account_journal j ON (l.journal_id=j.id)
                WHERE %s
                --GROUP BY l.id, l.account_id, l.date, j.code, l.currency_id, l.debit_currency, l.credit_currency, l.ref, l.name, m.id, m.name, c.rounding, cc.rounding, cc.position, c.position, c.symbol, cc.symbol, p.name,p.vat, anl.name
                ORDER BY %s
            ...
            for row in cr.dictfetchall():
                if data.get('display_accounts') == 'balance_not_zero' and currency.is_zero(row['debit'] - row['credit']):
                    ...
                    row['initial_bal'] = False
                    move_lines[account.code]['lines'].append(row)
                    move_lines[account.code]['tax_base_amount'] = row['tax_base_amount']
    ...
    def get_filters(self, default_filters={}):
        ...
        filter_dict = {
            ...
            'tax_view' : self.view_tax_base_amount
        }
    ...
    def action_pdf(self):
        ...
        _logger.error("action_pdf")
        return self.env.ref(
            'account_dynamic_reports'
            '.action_print_general_ledger').with_context(landscape=False).report_action(
    ...
    def get_xlsx_report(self, data, response):
        ...
        # Get record and data
        record = self.env['ins.general.ledger'].browse(
        ...
        row_pos_2 = 0
        row_pos = 0
        sheet.merge_range(0, 0, 0, 8, 'General Ledger' +
        ...
        if filter.get('include_details', False):
            sheet.write_string(row_pos, 0, _('Cuenta'), format_header)
            sheet.write_string(row_pos, 1, _('Fecha'), format_header)
            sheet.write_string(row_pos, 2, _('Diario-code'), format_header)
            sheet.write_string(row_pos, 3, _('Nit/ID'), format_header)
            sheet.write_string(row_pos, 4, _('Tercero'), format_header)
            sheet.write_string(row_pos, 5, _('Referencia'), format_header)
            sheet.write_string(row_pos, 6, _('Numero'), format_header)
            sheet.write_string(row_pos, 7, _('Nota'), format_header)
            sheet.write_string(row_pos, 8, _('Cuentas analítica'), format_header)
            sheet.write_string(row_pos, 9, _('Base Impuesto'), format_header)
            sheet.write_string(row_pos, 10, _('Debito'), format_header)
            sheet.write_string(row_pos, 11, _('Credito'), format_header)
        ...
        if account_lines:
            for line in account_lines:
                ...
                sheet.write(row_pos, 9, float(
                    account_lines[line].get('tax_base_amount')), line_header)
                sheet.write(row_pos, 10, float(
                    account_lines[line].get('debit')), line_header)
                sheet.write(row_pos, 11, float(
                    account_lines[line].get('credit')), line_header)
                sheet.write(row_pos, 12, float(
                    account_lines[line].get('balance')), line_header)

                if filter.get('include_details', False):
                    account_id = account_lines[line].get('id')
                    _logger.info('\nANTESSSSS\n %r \n\n', account_id)
                    count, offset, sub_lines, tax_view = record.build_detailed_move_lines(offset=0, account=account_id,
                                                                                fetch_range=1000000)
                    _logger.info('\n\n %r \n\n', [count, offset, sub_lines, tax_view])
                    for sub_line in sub_lines:
                        if sub_line.get('move_name') == 'Initial Balance':
                            row_pos += 1
                            sheet.write(row_pos, 6, sub_line.get('move_name'), line_header_light_initial_bold)
                            sheet.write(row_pos, 9, float(sub_line.get('tax_base_amount')), line_header_light_initial)
                            ...
                            row_pos += 1 
                            sheet.write(row_pos, 0, account_lines[line].get('name'), line_header_left)
                            ...
                            sheet.write(row_pos, 3, sub_line.get('partner_vat') or '', line_header_light)
                            sheet.write(row_pos, 4, sub_line.get('partner_name') or '', line_header_light)
                            sheet.write(row_pos, 5, sub_line.get('move_ref') or '', line_header_light)
                            sheet.write(row_pos, 6, sub_line.get('move_name'), line_header_light)
                            sheet.write(row_pos, 7, sub_line.get('lname') or '', line_header_light)
                            sheet.write(row_pos, 8, sub_line.get('analytic_account') or '', line_header_light)
                            sheet.write(row_pos, 9, float(sub_line.get('tax_base_amount')), line_header_light)
                            ...
                            row_pos += 1
                            sheet.write(row_pos, 6, sub_line.get('move_name'), line_header_light_ending_bold)
                            sheet.write(row_pos, 9, float(account_lines[line].get('tax_base_amount')), line_header_light_ending)
