            ...
            for spick in self.picking_ids:
                ...
                if (spick.move_line_ids):
                    ...
                    if (len(spick.move_line_ids)>=1):
                        ...
                        try:
                            _logger.info("calling button_validate")
                            bv_res = spick.button_validate()
                            if bv_res is not True:
                                res = { 'error': str(bv_res) }
                                _logger.error("stock pick button_validate/action_done error > " +str(res)+ " Entrega: "+str(spick) )
                                if ("expiry.picking.confirmation" in str(bv_res)):
                                    ac_res = spick.action_confirm()
                                    _logger.info("action_confirm res:"+str(ac_res))
                                    if ac_res is True:
                                        continue;
                            else:
                                continue;
                        except Exception as e:
                            _logger.error("stock pick button_validate/action_done error > "+str(e)+" Entrega: "+str(spick.name))
                            res = { 'error': str(e) }
                            pass;

                        try:
                            _logger.info("validating with assign")
                            spick.action_assign()
                            bv_res = spick.button_validate()
                            if bv_res is not True:
                                res = { 'error': str(bv_res) }
                            ...
                            _logger.error("stock pick action_assign/button_validate/action_done error > "+str(e)+" Entrega: "+str(spick.name))
                            res = { 'error': str(e) }
                            pass;
