...
class MercadoLibreConnectionBindingProductTemplate(models.Model):
                    ...
                    for variant in productT.product_variant_ids:
                        ...
                        if ( conditions_ok or variant_principal==False):
                            ...
                            if (variant_principal==False):
                                ...
                                ret = variant_principal.with_context(custom_context).product_post( meli=meli, config=config )
                                if (ret and len(ret) and 'name' in ret[0]):
                                    if first_product_post and bindT:
    ...
    def copy_from_rjson( self, rjson, meli=None ):
        ...
        for bindT in self:
            ...
            #basic info
            catid, wwwid = self.env["mercadolibre.category"].meli_get_category( rjson['category_id'], meli=meli, create_missing_website=config.mercadolibre_create_website_categories, config=config )
    ...
    def product_template_rebind( self, unbind_template=True ):
        ...
        for bindT in self:
            ...
            _logger.info("Rebinding "+str(conn_id))
            ...
            if product_template and conn_id:
                _logger.info("Binding again... connection_account:"+str(connection_account)+" product_template:"+str(product_template)+" conn_id:"+str(conn_id))
...
class MercadoLibreConnectionBindingProductVariant(models.Model):
    ...
    def product_get_meli_update( self ):
        ...
        for bindv in self:
            ...
            if bindv.meli_id and not meli.need_login():
                response = meli.get( "/items/"+bindv.meli_id, { 'access_token': meli.access_token } )
                rjson = response.json()
                if rjson and "error" in rjson:
                    ...
                    if "permalink" in rjson:
                        ...
                        ML_permalink_edit = company.get_ML_LINK_URL(meli=meli)+str("publicaciones/")+str(bindv.meli_id)+str("/modificar")
                        ML_permalink_api = str("https://api.mercadolibre.com/items/")+str(bindv.meli_id)+str("?include_attributes=all&access_token="+str(meli and meli.access_token))
                    if "error" in rjson:
                        ML_status = rjson["error"]
                        ML_permalink = ""
                    if "sub_status" in rjson:
...
#    meli_pub_variant_attributes = fields.Many2many(prod_att_line, relation='meli_pub_variant_attributes',column1='ml_template_id',column2='att_line_id', string='Atributos a publicar en ML',help='Seleccionar los atributos a publicar')
        ...
        for bind in self:
            ...
            if notify:
                if (bind.meli_stock_status == 'revision_error' and bind.connection_account.configuration.mercadolibre_seller_user):
                    ...
                    bodymess = "Revisar la publicacion "+bind.conn_id+"\n"+str(bind.stock_error)+"\n"+str(url)
                    if "Internal Server Error" in str(bind.stock_error):
                        bind.meli_stock_status == 'update'
                    else:
                        bind.connection_account.configuration.mercadolibre_seller_user.partner_id.message_post(body=str(bodymess),message_type=product_message_type)
    ...
    def _meli_stock_resume(self):
        ...
        for bind in self:
            ...
            stocks_str, stocks_on_hand, stocks_available, stocks_meli = bind.get_stock_str(meli=meli,config=config)
    ...
    def product_post_stock( self, context=None, meli=None, optimize=False ):
        ...
        for bindv in self:
            ...
            #
            try:
                #_logger.info("mercadolibre.product product_post_stock "+str(meli_id))
                res = {}
