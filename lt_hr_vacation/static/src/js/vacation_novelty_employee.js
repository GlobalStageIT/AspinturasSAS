odoo.define('lt_hr_vacation.dashboard.view_custo', function(require) {
    'use strict';
    ...
    var TimeOffCalendarController = CalendarController.extend({
        events: _.extend({}, CalendarController.prototype.events, {
            ...
            'click .btn-vacation-novelty': '_onNewVacationNovelty',
        }),
        ...
        renderButtons: function ($node) {
            ...
            $(QWeb.render('hr_holidays.dashboard.calendar.button', {
                ...
                vacation_novelty: _t('Compensated Vacation'),
        ...
        _getContractContext: function(employee) {
            return this._rpc({
                model: 'hr.contract',
                method: 'search_read',
                domain: [['employee_id', '=', employee], ['state', '=', 'open']],
                fields: ['id'],
                ...
                if (ContractData.length === 0) {
                    ...
                    return ContractData[0]['id'];
                }
            });
        },

        _getEmployeeContext: function() {
            return this._rpc({
                model: 'hr.employee',
                method: 'search_read',
                domain: [['user_id', '=', this.context.uid]],
                fields: ['id'],
                limit: 1,  // Retrieve only one record
            }).then(function(EmployeeData) {
                return EmployeeData[0]['id'];
            });
        },

        _getNoveltyTypeContext: function() {
            return this._rpc({
                model: 'hr.novelty.type',
                method: 'search_read',
                domain: [['is_vacation_compensated', '=', true]],
                limit: 1,  // Retrieve only one record
            }).then(function(noveltyTypeData) {
                return noveltyTypeData[0].id;
            });
        },

        _onNewVacationNovelty: function () {
            let self = this;

            self._rpc({
                model: 'ir.ui.view',
                method: 'get_view_id',
                args: ['lt_hr_vacation.view_hr_vacation_novelty_form'],
            }).then(async function(ids) {
                const noveltyTypeId = await self._getNoveltyTypeContext(self);
                const employeeId = await self._getEmployeeContext(self);
                const contractId = await self._getContractContext(employeeId);
                self.noveltyDialog = new dialogs.FormViewDialog(self, {
                    res_model: "hr.novelty",
                    view_id: ids,
                    context: {
                        'default_novelty_type_id': noveltyTypeId,
                        'default_employee_id': employeeId,
                        'default_contract_id': contractId,
                        'default_company_id': self.context.company,
                        'default_state': 'pending_approval',
                    },
                    title: _t("Compensated Vacation"),
                    disable_multiple_selection: true,
                    on_saved: function() {
                        self.reload();
                    },
                });
                self.noveltyDialog.open();
            });
        },
